...
# Planning outputs use small enum surfaces so local structured generation has fewer invalid values to invent.
...
class PlannedTask(BaseModel):
    ...
    Planning tasks are intentionally explicit about dependencies, risks, and
    acceptance criteria because the QA Agent validates these fields later.
    """
...
class PlanResult(BaseModel):
    ...
    The TaskPlanBuilderTool normalizes this schema before QA reads it, so the
    plan can be generated by an LLM while still keeping deterministic ordering.
    """
